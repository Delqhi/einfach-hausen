...
const checks={"/agb":["Allgemeine Geschäftsbedingungen","0 % Vermittlungsprovision","Impressum"],"/datenschutz":["Deine Hausdaten gehören dir","Rechte betroffener Personen","AGB"],"/impressum":["Anbieterkennzeichnung","Gina Schulze","Urheberrecht"],"/sicherheit":["Nichts passiert","Keine automatische Beauftragung","Garantie"],"/barrierefreiheit":["zugänglich bedeuten","Tastatur","Konformitätsbehauptung"]};
const res=[];
for(const [route,need] of Object.entries(checks)){
 await p.goto("http://127.0.0.1:4197"+route,{waitUntil:"networkidle",timeout:90000});
 ...
 for(const w of [390,1440]){
  ...
  await p.screenshot({path:"docs/brand/evidence/05-web/legal-"+route.slice(1)+"-"+w+".png",fullPage:true});
  ...
  res.push({route,width:w,overflow:m.overflow,small:m.small,axe});
  console.log(route,w,JSON.stringify({overflow:m.overflow,small:m.small,axe}));
 }
}
writeFileSync("docs/brand/evidence/05-web/legal-check.json",JSON.stringify({errors:errs,result:res},null,2));
if(errs.length)throw Error(errs.join("|").slice(0,300));
console.log("LEGAL_OK");
await b.close();
