...
for (const [name, path] of ROUTES) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const heads = all('h1, h2').slice(0, 6);
    ...
    const lowContrast = heads.filter(h => {
      ...
      if (bgImage) {
        const stops = [...bgImage.matchAll(/rgba?\(([^)]+)\)/g)].map(m => `rgb(${m[1]})`);
        if (stops.length) {
  ...
  if (metrics.lowContrast > 0) addFinding(name, 'critical', 'low-contrast', `${metrics.lowContrast} Überschriften unter 4.5:1`);
}
