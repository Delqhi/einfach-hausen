...
export async function GET(_req:NextRequest,{params}:{params:Promise<{id:string}>}){
  const u=await getCurrentUser(); if(!u)return new NextResponse('Unauthorized',{status:401}); const {id}=await params;
  ...
  let allowed=u.id===d.homeowner_id;
  ...
  const root=path.resolve(process.cwd(),'data','private'); const file=path.resolve(root,d.path); if(!file.startsWith(root+path.sep))return new NextResponse('Invalid path',{status:400});
