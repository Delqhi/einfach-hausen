...
import { EHPanel, EHList, EHErrorState, EHField, EHTextarea } from '@/design-system';
...
export default async function ProProfile({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="provider" active="/pro/profile" title="Profil & Vertrauen" subtitle={p?.business_name||ctx.businessName}>
    ...
    {sp.stripe==='owner'&&<EHErrorState text="Auszahlungen kann nur der Firmeninhaber einrichten." />}

    <EHPanel title={p?.verified?'Unternehmen geprüft':'Unternehmensprüfung erforderlich'}><p>{p?.verified?'Identität und eingereichte Unternehmensnachweise sind geprüft.':v?`Prüfstatus: ${statusLabel(v.status)}`:'Gewerbe-, Qualifikations- und Versicherungsnachweise müssen geprüft werden.'}</p>{v?.admin_note&&<small>Rückmeldung: {v.admin_note}</small>}</EHPanel>
        {ctx.isOwner&&!p?.verified&&<EHPanel title="Nachweise einreichen"><form action={submitProviderVerificationAction}><EHField id="prov-doc" label="Nachweis" hint="PDF, JPG, PNG oder WebP bis 12 MB."><input id="prov-doc" type="file" name="document" accept="application/pdf,image/jpeg,image/png,image/webp" required/></EHField><EHField id="prov-note" label="Hinweis"><EHTextarea id="prov-note" name="note" rows={3} placeholder="Gewerbeanmeldung, Meister-/Qualifikationsnachweis, Versicherung …"/></EHField><button className="btn primary">Zur Prüfung einreichen</button></form></EHPanel>}

    <SectionTitle>Firmenprofil & Leistungen</SectionTitle><EHList label="Firmenprofil" items={[{ id: 'wizard', title: 'Firmendaten-Wizard', text: 'Firmendaten, Leistungen und Arbeitsgebiet strukturiert pflegen', href: '/pro/onboarding' }]} /><SectionTitle>Partnervertrag & Standards</SectionTitle><EHPanel title={activation.receivesNewJobs?'Aktiver Einfach-Hausen-Vertragspartner':(p?.contract_status==='active'?'Freigabe unvollständig':`Vertragsstatus: ${statusLabel(p?.contract_status||'pending')}`)}>{activation.receivesNewJobs?<BadgeCheck/>:<FileCheck2/>}<div><p>Neue Anfragen gibt es nur bei freigegebener Unternehmensprüfung, aktivem Vertrag und vollständig bestätigten Qualitätschecks. Das Qualitätsmatching ist unabhängig vom gebuchten Partner-Tarif.</p><div className="contract-checks"><span className={activation.insuranceVerified?'ok':''}>{activation.insuranceVerified?'✓':'○'} Versicherung</span><span className={activation.qualificationVerified?'ok':''}>{activation.qualificationVerified?'✓':'○'} Qualifikation</span><span className={activation.contractVerified?'ok':''}>{activation.contractVerified?'✓':'○'} Partnervertrag</span><span className={activation.qualityStandardVerified?'ok':''}>{activation.qualityStandardVerified?'✓':'○'} Qualitätsstandard</span></div>{activation.missing.length>0&&<small>Noch offen: {activation.missing.join(' · ')}</small>}{activation.receivesNewJobs&&<small>0 % Provision · Reaktionsziel {p.response_target_minutes} Min.</small>}</div></EHPanel>
