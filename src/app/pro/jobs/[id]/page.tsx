...
import { ProviderSectionHeader } from '@/components/provider/workspace';
...
export default async function ProJob({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="provider" active={isAccepted?'/pro/orders':'/pro'} title={isContact?'Kontaktanfrage':isAccepted?'Auftrag':'Anfrage'} subtitle={access.category}>
    ...
    <div className="detail-head pro-detail"><span className={`status ${access.status}`}>{isContact?(isAccepted?'Verbunden':'Kontakt gesucht'):statusLabel(access.status)}</span><h1>{access.title.replace(/^Ansprechpartner:\s*/,'')}</h1><p>{access.description}</p><div className="meta-line"><span><MapPin/>{isAccepted&&access.address?access.address:access.postcode}</span>{!isContact&&<span><CalendarDays/>{dateLabel(access.preferred_date)}</span>}</div>{access.photo_id&&<JobMedia src={`/api/job-media/${access.photo_id}`} alt="Foto, Video oder Sprachnachricht zum Thema" kind={mediaKindFromPath(access.photo_path)}/>}{!isContact&&<div className="budget-line"><small>Richtpreis</small><strong>{access.budget_min&&access.budget_max?`${euro(access.budget_min)} – ${euro(access.budget_max)}`:euro(access.budget_max)}</strong></div>}</div>
    ...
    {!isAccepted&&ctx.canManageJobs&&access.status!=='completed'&&isContact&&<>
      ...
      <ProviderSectionHeader title="Ansprechpartner anbieten" description="Wähle die konkrete Person, die den Eigentümer fachlich übernimmt." />
    ...
    {isAccepted&&<>
      ...
      <ProviderSectionHeader title="Ansprechpartner" description="Diese Person ist für den Eigentümer sichtbar und betreut den Vorgang direkt." />
      ...
      {mine&&!isContact&&<><ProviderSectionHeader title="Rechnungen" description="Rechnungen bleiben am Auftrag und in der Hausakte nachvollziehbar." />{invoices.length>0&&<div className="stack pro-doc-list">{invoices.map(inv=><a key={inv.id} href={`/pro/invoices/${inv.id}`}><strong>{inv.invoice_number} · {euro(inv.total_gross)}</strong><small>{invoiceStatusLabel(inv.status)} · fällig {dateLabel(inv.due_date)}</small></a>)}</div>}<InvoiceForm jobId={access.id} defaultAmount={quote?.amount||access.budget_max||0} primary={access.status==='completed'}/><ProviderSectionHeader title="Weitere Dokumente" description="Leistungsnachweise, Garantien und weitere Unterlagen zum Auftrag." />{docs.length>0&&<div className="stack pro-doc-list">{docs.map(d=><a key={d.id} href={`/api/documents/${d.id}`} target="_blank" rel="noreferrer"><strong>{d.title}</strong><small>{d.kind}</small></a>)}</div>}<DocumentForm jobId={access.id}/></>}
    </>}
  </AppShell>;
}
