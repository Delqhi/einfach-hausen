...
export function InvoiceForm({jobId,defaultAmount,primary=false}:{jobId:number;defaultAmount:number;primary?:boolean}){
  return <form action={createInvoiceAction.bind(null,jobId)} className="invoice-form">
    ...
    <div className="invoice-line"><label>Leistung<input name="itemDescription" defaultValue="Ausgeführte Handwerkerleistung" required/></label><label>Menge<input name="itemQuantity" type="number" step="0.01" min="0.01" defaultValue="1" required/></label><label>Einheit<input name="itemUnit" defaultValue="pauschal" required/></label><label>Netto €<input name="itemPrice" type="number" step="0.01" min="0" defaultValue={(defaultAmount/1.19/100).toFixed(2)} required/></label><label>MwSt. %<select name="itemTax" defaultValue="19"><option value="19">19 %</option><option value="7">7 %</option><option value="0">0 %</option></select></label></div>
