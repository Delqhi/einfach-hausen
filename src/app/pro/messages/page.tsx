...
export default async function Messages({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="provider" active="/pro/messages" title="Nachrichten" subtitle="Deine direkten Kundenkontakte">
    {customers.length===0?<div className="empty dark-empty"><MessageSquare/><strong>Noch keine direkten Kontakte</strong><p>Sobald dir ein gebuchter Auftrag zugewiesen wurde, bleibt der Kunde nach Abschluss als direkter Kontakt erhalten.</p></div>:<>
