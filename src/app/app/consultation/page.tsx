...
export default async function Consultation({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="homeowner" active="/app" title="Beratung" subtitle="Erst einen Fachmann fragen – ohne Auftrag">
    ...
    <div className="consultation-points"><span><UserRound/><b>Konkreter Ansprechpartner</b><small>Du weißt, mit wem du sprichst.</small></span><span><ShieldCheck/><b>Geprüfter Partner</b><small>Passend zu deinem Thema und deiner Region.</small></span><span><Camera/><b>Foto möglich</b><small>Hilft bei einer ersten Einschätzung.</small></span></div>
    <form action={createConsultationAction} className="consultation-form"><label>Wobei brauchst du Rat?<textarea name="description" rows={6} required placeholder="Zum Beispiel: Mein Dach ist an einer Stelle feucht. Was könnte die Ursache sein?"/></label><label>Foto <small>(optional)</small><input type="file" name="photo" accept="image/*"/></label><button className="btn primary wide">Ansprechpartner finden</button></form>
  </AppShell>;
}
