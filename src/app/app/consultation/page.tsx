...
import { EHAppHeader, EHPanel, EHErrorState, EHField, EHTextarea, EHInput } from '@/design-system';
...
export default async function Consultation({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  return <AppShell role="homeowner" active="/app" title="Beratung" subtitle="Erst einen Fachmann fragen – ohne Auftrag">
    ...
    <form action={createConsultationAction}>
      ...
      <EHField id="con-photo" label="Foto oder Video (optional)" hint="JPEG, PNG, WebP oder HEIC bis 8 MB; MP4, WebM, MOV oder M4V bis 25 MB."><input id="con-photo" type="file" name="photo" accept="image/jpeg,image/png,image/webp,image/heic,video/mp4,video/webm,video/quicktime,video/x-m4v"/></EHField>
      <button type="submit">Ansprechpartner finden</button>
    </form>
    </EHPanel>
