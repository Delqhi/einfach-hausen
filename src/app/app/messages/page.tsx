...
export default async function Messages({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="homeowner" active="/app/messages">
    <div className="page-head"><h1 className="page-title">Meine Ansprechpartner</h1><p className="page-subtitle">Menschen, die dein Haus bereits kennen. Du kannst direkt fragen, schreiben oder später eine Arbeit abstimmen — ohne jedes Mal neu suchen zu müssen.</p></div>
    {contacts.length===0?<div className="empty"><UserRound/><strong>Noch keine Ansprechpartner</strong><p>Sobald du einen passenden Menschen verbinden lässt oder einen Auftrag buchst, speichert Einfach Hausen deinen konkreten Ansprechpartner automatisch.</p></div>:<>
