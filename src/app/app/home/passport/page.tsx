...
export default async function HousePassport(){
  const user=await requireUser('homeowner'); const property=primaryProperty(user.id); if(!property)return <main className="passport-page"><div className="owner-route-state owner-passport-empty"><House aria-hidden="true"/><span className="owner-state-kicker">Digitaler Hauspass</span><h1>Deine Hausakte ist noch nicht eingerichtet.</h1><p>Lege zuerst dein Zuhause an. Danach entsteht hier der Hauspass mit Technik und dokumentierter Historie.</p><Link className="owner-state-action" href="/app/home">Mein Haus einrichten</Link></div></main>;
