...
export default async function MyHome(){
  ...
  return <AppShell role="homeowner" active="/app/home" title="Mein Haus" subtitle="Deine digitale Hausakte">
    ...
    <section id="technik"><SectionTitle>Technik & Ausstattung</SectionTitle><div className="asset-grid mobile-assets">{assets.map(a=><article key={a.id}><SolarPanel/><div><strong>{a.name}</strong><small>{a.kind}{a.installed_year?` · ${a.installed_year}`:''}</small><p>{a.details}</p></div></article>)}{assets.length===0&&<div className="empty compact"><p>Noch keine Technik hinterlegt. Mit einem ersten Eintrag kann dein Hausmeister Wartungen und Hauswissen besser einordnen.</p></div>}</div>
    <form action={addHouseAssetAction} className="asset-form" aria-label="Technik zur Hausakte hinzufügen"><label><span>Bereich</span><select name="kind" required defaultValue=""><option value="" disabled>Technik / Bereich</option><option value="heating">Heizung / Wärmepumpe</option><option value="pv">PV-Anlage</option><option value="storage">Batteriespeicher</option><option value="wallbox">Wallbox</option><option value="roof">Dach / Dachrinne</option><option value="windows">Fenster / Türen</option><option value="garden">Garten</option><option value="smarthome">Smart Home / Sicherheit</option></select></label><label><span>Bezeichnung</span><input name="name" placeholder="z. B. Wärmepumpe Keller" required/></label><label><span>Installationsjahr <small>(optional)</small></span><input name="installedYear" type="number" placeholder="z. B. 2024"/></label><label><span>Modell / Hinweise <small>(optional)</small></span><input name="details" placeholder="z. B. Hersteller und Modell"/></label><button className="btn ghost"><Plus size={16} aria-hidden="true"/>Hinzufügen</button></form></section>
