...
import { Building2,CalendarDays,CheckCircle2,ChevronRight,FileText,House,NotebookPen,Plus,Settings2,SolarPanel } from 'lucide-react';
...
export default async function MyHome(){
  ...
  const tasks=db.prepare("SELECT * FROM maintenance_tasks WHERE homeowner_id=? AND status='open' ORDER BY due_date LIMIT 8").all(u.id) as any[];
  ...
  return <AppShell role="homeowner" active="/app/home" title="Mein Haus" subtitle="Deine digitale Hausakte">
    ...
    <div className="house-menu">
      ...
      <Link href="/app/documents"><span className="house-menu-icon"><FileText/></span><span className="grow"><strong>Dokumente</strong><small>{docs.c} Dokumente in deiner Hausakte</small></span><ChevronRight/></Link>
