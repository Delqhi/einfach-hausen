...
export default async function Documents(){
  ...
  return <AppShell role="homeowner" active="/app/documents"><h1 className="page-title">Dokumente</h1><p className="page-subtitle">Rechnungen, Leistungsnachweise und Zahlungsbelege an einem Ort.</p><div className="stack">{uploaded.map(d=><a className="document-row" href={`/api/documents/${d.id}`} target="_blank" rel="noreferrer" key={`d-${d.id}`}><FileText/><div><strong>{d.document_title}</strong><small>{d.business_name||'Mein Hausmeister'} · {d.job_title}</small></div><span>{d.kind}</span></a>)}{payments.map(p=><Link className="document-row" href={`/app/documents/${p.job_id}/receipt`} key={`p-${p.id}`}><ReceiptText/><div><strong>Zahlungsbeleg · {p.title}</strong><small>{p.business_name} · {euro(p.amount)}</small></div><ShieldCheck/></Link>)}{uploaded.length===0&&payments.length===0&&<div className="empty"><FileText/><strong>Noch keine Dokumente</strong><p>Rechnungen und Belege erscheinen automatisch nach der Abwicklung.</p></div>}</div></AppShell>;
}
