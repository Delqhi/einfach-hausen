...
export default async function Documents(){
  ...
  return <AppShell role="homeowner" active="/app/documents">
    ...
    {!empty && <EHWorkSection title={`Rechnungen · ${invoices.length}`}><EHDossierList label="Rechnungen" items={invoices.map(i=>({id:`i-${i.id}`,kind:'Rechnung',title:`Rechnung ${i.invoice_number}`,detail:`${i.business_name} · ${i.title}`,amount:euro(i.total_gross),href:`/app/invoices/${i.id}`,status:<EHStatus>{invoiceStatusLabel(i.status)}</EHStatus>}))}/></EHWorkSection>}
    {!empty && <EHWorkSection title={`Nachweise & Unterlagen · ${uploaded.length}`}><EHDossierList label="Hochgeladene Dokumente" items={uploaded.map(d=>({id:`d-${d.id}`,kind:d.kind,title:d.document_title,detail:`${d.business_name||'Einfach Hausen'} · ${d.job_title}`,href:`/api/documents/${d.id}`}))}/></EHWorkSection>}
    {!empty && <EHWorkSection title={`Zahlungsbelege · ${payments.length}`}><EHDossierList label="Zahlungsbelege" items={payments.map(p=>({id:`p-${p.id}`,kind:'Zahlungsbeleg',title:p.title,detail:p.business_name,amount:euro(p.amount),href:`/app/documents/${p.job_id}/receipt`}))}/></EHWorkSection>}
