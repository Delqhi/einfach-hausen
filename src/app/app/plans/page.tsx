...
import { EHAppHeader, EHPanel, EHList, EHErrorState } from '@/design-system';
...
export default async function Plans({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="homeowner" active="/app/plans">
    ...
    {sp.error&&<EHErrorState text={sp.error} />}{sp.checkout==='success'&&<div className="alert success" role="status" aria-live="polite">Buchung erfolgreich.</div>}{sp.checkout==='processing'&&<div className="alert success" role="status" aria-live="polite">Zahlung eingegangen. Aktivierung folgt erst nach bestätigtem Stripe-Webhook.</div>}{sp.checkout==='unavailable'&&<EHErrorState text="Onlinezahlung ist derzeit nicht vollständig konfiguriert. Es wurde kein Zahlungs- oder Tarifstatus geändert. Du kannst den Tarif später erneut wählen." />}
    ...
    {isPilot && <div className="alert success" role="status">Pilot-Vorteil aktiv: 15% Dauer-Vorteil auf Pakete und Mitgliedschaften — wird beim Checkout automatisch angewendet.</div>}
    <SectionTitle>Mitgliedschaften</SectionTitle>{plans.map((p,i)=><EHPanel key={p.slug} title={`${p.title}${p.slug==='plus'?' (empfohlen)':''} — ${euro(p.monthly_amount)}/Monat`}><p>{p.description}</p><EHList label={p.title} items={[{ id: p.slug + '-base', title: 'Zentrale Hausorganisation & Hilfe' },{ id: p.slug + '-partner', title: 'Geprüfte Vertragspartner' },{ id: p.slug + '-vergleich', title: 'Angebots- und Preisvergleich' },{ id: p.slug + '-akte', title: 'Digitale Hausakte' },...(p.annual_house_check?[{ id: p.slug + '-check', title: 'Jährlicher Haus-Check' }]:[]),...(p.partner_discount_bps?[{ id: p.slug + '-rabatt', title: `Bis zu ${(p.partner_discount_bps/100).toFixed(0)} % vertraglicher Kundenvorteil` }]:[])]} /><form action={startMembershipCheckoutAction.bind(null,p.slug)}><button disabled={current?.status==='active'&&current.plan_slug===p.slug}>{current?.status==='active'&&current.plan_slug===p.slug?'Aktiv':`${p.title} wählen`}</button></form></EHPanel>)}
    <SectionTitle>Jahres- & Premiumpakete</SectionTitle>{packages.map((p:any)=><EHPanel key={p.slug} title={`${p.title} — ${euro(p.price_amount)}`}><p>{p.description}</p><EHList label={p.title} items={JSON.parse(p.services_json).map((x:string)=>({ id: p.slug + '-' + x.slice(0, 12), title: x }))} /><form action={purchasePackageAction.bind(null,p.slug)}><button>Paket buchen</button></form></EHPanel>)}
