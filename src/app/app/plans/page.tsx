...
export default async function Plans({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="homeowner" active="/app/plans">
    <div className="plans-hero"><Crown/><div><h1>Mehr Haus. Weniger Aufwand.</h1><p>Mitgliedschaft oder Jahrespaket: Dein KI-Hausmeister plant, vergleicht und organisiert — du behältst nur die Entscheidungen.</p></div></div>
    {sp.error&&<div className="alert error">{sp.error}</div>}{sp.checkout==='success'&&<div className="alert success">Buchung erfolgreich.</div>}
