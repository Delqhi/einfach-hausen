...
export default async function Plans({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <AppShell role="homeowner" active="/app/plans">
    ...
    {sp.error&&<div className="alert error" role="alert">{sp.error}</div>}{sp.checkout==='success'&&<div className="alert success" role="status" aria-live="polite">Buchung erfolgreich.</div>}{sp.checkout==='processing'&&<div className="alert success" role="status" aria-live="polite">Zahlung eingegangen. Aktivierung folgt erst nach bestätigtem Stripe-Webhook.</div>}{sp.checkout==='unavailable'&&<div className="alert error" role="alert">Onlinezahlung ist derzeit nicht vollständig konfiguriert. Es wurde kein Zahlungs- oder Tarifstatus geändert. Du kannst den Tarif später erneut wählen.</div>}
