**Updated:** 2026-09-01 22:49Z — **Operator-Weisung: neues Design (aktuelles nicht gewünscht). ChatGPT-Web-Session gelesen (WEB:4314917c / "Design Auftrag einfach Hausen", via OCI sin-gpt-web-browser CDP :9223):** Premium-Consumer-Redesign-Spec ist vom Operator APPROVED (DoktorABC-inspirierte Art-Direction; Macro-Composition vor Micro-Polish; 3 Wellen: komplette öffentliche Website → App-Polish mit eingefrorener IA → sin-frontend-design RED/GREEN). Spec vollständig extrahiert nach **docs/design/PREMIUM_CONSUMER_REDESIGN_SPEC.md** (9ec847b). **Sicherheitsrefs angelegt und gepusht: backup/pre-premium-redesign-20260902 = aa84ee2, design/premium-consumer-v1 = aa84ee2** (Worktree folgt). Rollback-Vertrag: main bleibt unangetastet bis visuelle Freigabe; kein reset --hard/clean/force-push. Kanonischer Task **T-0210** (critical, claimed) angelegt. ChatGPT-Seite blockte am fehlenden Mac-M1-Zugriff — wir auf OCI haben repo+CI, daher läuft die Umsetzung hier. T-0120 Security-Fuzz bleibt claimed, pausiert hinter dem Redesign. Genau eine nächste Aktion: **T-0210 Phase 1+2: Worktree für design/premium-consumer-v1 aufsetzen und Homepage-Premium-Redesign nach Spec starten** (Asset-Strategie zuerst, siehe Spec §7/§16).

**Updated:** 2026-09-01 22:14Z — **T-0119 Performance (Server Actions, SQLite, Index-Hot-Path-Audit) DONE & geschlossen (Taskplan done=94, render+validate PASS).** Root-Causes an der Produktions-DB-Kopie (155.406 crm_leads, 127MB WAL) gemessen und behoben: (1) `syncCrmLifecycle` lief **2x vollstaetigen `SELECT *`-Collapse (~3,2-3,9s) pro /admin/crm Render** → wasserzeichen-basiertes Inkrement (`crm_meta.lifecycle_full_sync_at`, `datetime('now')` via ON CONFLICT): Render-Syncs collapsen nur `updated_at>=Wasserzeichen` mit schmalem Identitaets-Scan, Lazy-Full-Row-Hydration nur fuer echte Duplikat-Gruppen, Lazy-Prior-Seed (leeres Fenster ueberspringt den 155k-Seed), Skip-Guard (2. Collapse nur wenn User-Loop aenderte); Forschungsimport behaelt den vollen Pass. (2) `matchCandidates` OR-Query degenerierte zu vollem Autoindex-Scan (0,5-0,8s/User) **und die Partial-Identity-Indizes (WHERE col!='') wurden vom SQLite-Planner nie gewaehlt ('no query solution')** → volle Indizes + per-clause Lookups. (3) `idx_crm_updated_at` fuer Fensterscans; (4) db.ts `synchronous=NORMAL` + `wal_autocheckpoint=1000` (127MB-WAL beobachtet). Messung Prod-Kopie: steady sync **3,3s → 2-4ms**, recent-Sync bei 155k **3976→1262ms**, matchCandidates **→ <1ms**. Semantik-Beweise: Cross-Watermark-Merge PASS (Prior-Seed ueber Grenze), Idempotenz PASS, Insert-Dedupe PASS. Betriebsinfo: erster Boot pro Prozess zahlt einmalig ~1-2s refreshNormalizedIdentity; Debug-Anomalie (proof3 deduplicated=0) war Test-Seed-Bug, nicht Implementierung. Gates: **test:crm 20/20**, fullflow e2e **ok:true 15/15**, lint 0 Fehler (30 Alt-Warnungen), Build OK, Release-Gate **11/11**, Production-Smoke **17/17** @ https://einfachhausen.de. Commit **050ec6e** (rebased auf aa84ee2) gepusht. Genau eine naechste Aktion: **T-0120 Security: adversarial authorization/input fuzz regression wave** (highest eligible via `sin-gpt-web-state next`).

# Einfach Hausen — Canonical Next-Agent Contract

**Updated:** 2026-09-01 20:55Z — **T-0118 Performance CWV/Bundle DONE & geschlossen (Taskplan done=93, render+validate PASS).** Drei Root-Causes behoben und gemessen: (1) InterVariable.woff2 352KB → 61KB (pyftsubset latin, ä/ö/ü/ß+Quotes verifiziert, opsz/wght-Achsen erhalten); (2) 247KB @supabase/ssr+realtime-Vendor (Chunk 0neozj40ud7c3) wurde eager via Root-Layout AuthContext in JEDE Route gezogen → `src/lib/supabase.ts` auf lazy `getSupabase()` umgestellt (eager-Export = throwing Proxy, Fail-Fast), 15 Call-Sites konvertiert inkl. neu geschriebener AuthContext; Chunk fehlt jetzt komplett im served Home-HTML `<head>` und lädt post-Hydration per dynamic import; (3) gsap SplitText+lenis in motion.tsx jetzt feature-scoped dynamic imports. Non-async Root-JS: / 480→209KB, /login 362→116KB, /register 346→100KB, /app 377→104KB; Transfer-Summen pro Route −40.1% (z.B. / 697→420KB, /login 713→421KB). **Bewiesene Regression im Verifikationslauf gefunden+gefixt:** das Refactor-Diff hatte `gsap.registerPlugin(ScrollTrigger, useGSAP)` entfernt → Laufzeit-Crash `iH is not a function`/Error-Page; Registration wiederhergestellt. Gates: lint 0 errors (30 Alt-Warnings), Production-Build OK (Build-Env T-0200), **Fullflow e2e ok:true 15/15** (lazy-Supabase-Client-Auth-Pad bewiesen, zero browser runtime errors), **Release-Gate --only=build,perf 5/5** (CLS 0.0000, Transfer 383KB ≤1.5MB, Load 2360ms ≤8s). Betriebsinfo: `gsap.registerPlugin(ScrollTrigger, useGSAP)` muss modular Top in motion.tsx bleiben — Entfernen bricht ScrollTrigger zur Laufzeit. Genau eine nächste Aktion: **T-0119 Performance-Remediation Fortsetzung** (highest eligible via `sin-gpt-web-state next`).

**Updated:** 2026-09-01 19:25Z — **T-0147 Browser-E2E Website DONE & geschlossen (Taskplan done=92, render+validate PASS).** Root-Cause des wiederholten Tech-Login-Timeouts gefunden und behoben — NICHT Last (Operator-These) und kein Produktdefekt: das Repo-`.next` (Build 17:28) war ohne `NEXT_PUBLIC_SUPABASE_URL/ANON_KEY` gebaut; der Client-Login lief still in den Fail-Soft-Proxy (`Supabase nicht konfiguriert`), nie ein GoTrue-`/token`-Call (GoTrue-Logs belegen: null /token-Requests der e2e-Identities, signup/admin-Events nur serverseitig). Neu gebaut mit `/etc/einfach-hausen-build.env` (T-0200-Policy, BUILD_ID gl7882827): **Fullflow scripts/e2e.mjs PASS ok:true, 15/15 checks, zero browser runtime errors** (19:18Z, Production-Build-Server, echte Supabase-Auth, register→login→Anfrage→Matching→Booking→Invoice→Transfer; Load 2.4-3.6 1-min arm64/4vCPU). Firefox+WebKit Website-Acceptance-Matrix PASS (18:30-18:45Z). Neuer Fail-Fast-Guard in `scripts/e2e.mjs`: bricht ab, wenn das Client-Bundle die Supabase-Origin nicht enthält (verhindert stille Supabase-los-Build-Reruns). **Betriebsinfo: Vor jedem `npm run build`/`test:e2e` Build-Env sourcen (`set -a; source /etc/einfach-hausen-build.env; set +a`) — sonst bricht der Client-Login still.** Ops: GoTrue-HTTP-Probe signup+password-grant 200 (gesund); e2e-Identity-Cleanup verifiziert (0 @example.test in GoTrue). Genau eine nächste Aktion: **T-0118 Performance: public and app CWV/bundle remediation** (highest eligible, `sin-gpt-web-state next`).
