...
## Kontext
- **NEU T-0211 (owner=chatgpt-web, backlog, high):** Operator will eine App-Promo/Newsletter-Sektion (NewsletterAppPromo: weisse Card, links Eyebrow+Typo+2 Store-Buttons, rechts Phone-Mockup, responsive) als LETZTE Sektion direkt ueber dem Footer auf der Startseite. Vollstaendige Referenzimplementierung (JSX+CSS) hat der Operator per Chat-Paste; Details+Kontrakte stehen im Task-Record (sin-gpt-web-state show T-0211). Umsetzung NACH T-0210-Merge auf main, niemals im eh-premium-redesign-Worktree.
