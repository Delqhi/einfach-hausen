...
const fs=load("fs"),path=load("path"),ts=load("typescript"),R=load("react"),e=R.createElement;
const root=path.resolve(__dirname,"../..");process.chdir(root);
load.extensions[".tsx"]=(m,f)=>m._compile(ts.transpileModule(fs.readFileSync(f,"utf8"),{compilerOptions:{jsx:ts.JsxEmit.ReactJSX,module:ts.ModuleKind.CommonJS,esModuleInterop:true}}).outputText,f);
load.extensions[".css"]=m=>{m.exports={__esModule:true,default:new Proxy({},{get:(_,key)=>String(key)})};};
const W=load(root+"/packages/eh-design/src/workspace.tsx"),P=load(root+"/packages/eh-design/src/primitives.tsx"),D=load(root+"/packages/eh-design/src/workspace-detail.tsx");
const I=load("lucide-react");
...
async function run(){const{chromium}=load("playwright-core");const b=await chromium.launch({executablePath:process.env.EH_PREVIEW_CHROME||"/home/ubuntu/.local/share/eh-brand-browser/chromium-1234/chrome-linux/chrome",headless:true,args:["--no-sandbox"]});const p=await b.newPage();const evidence=[];
for(const[name,scene]of Object.entries(scenes)){
 ...
 const markup=load("react-dom/server").renderToStaticMarkup(content).replaceAll("/brand/logo-full.png","data:image/png;base64,"+logo);
